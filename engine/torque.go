...
func AddSTTorque1(dst *data.Slice) {
	...
	if !DisableSlonczewskiTorque1 && !FixedLayer1.isZero() {
		...
		if rec {
			defer cuda.Recycle(vapp)
		}
		Jcurr := cuda.Buffer(vapp.NComp(), vapp.Size())
		defer cuda.Recycle(Jcurr)
		if !DisableVoltageInt1 {
			a1int, rec := A1int1.Slice()
			if rec {
				defer cuda.Recycle(a1int)
			}
			a2int, rec := A2int1.Slice()
			if rec {
				defer cuda.Recycle(a2int)
			}
			JfromV(vapp, a1int, a2int, M.Buffer(), fl, Jcurr, ToMulFactorInt1)
			Jint1.AddTo(Jcurr)
		}
		j := cuda.ToMSlice(Jcurr)
		defer j.Recycle()
...
func JfromV(Vappl, A1, A2, m, refM, Jcurr *data.Slice, ToMulFactor bool) {
	...
	a1int := cuda.Buffer(1, A1.Size())
	a2int := cuda.Buffer(1, A1.Size())
	factor := cuda.Buffer(1, A1.Size())
	factor1 := cuda.Buffer(1, A1.Size())
	dp := cuda.Buffer(1, A1.Size())
	defer cuda.Recycle(a1int)
	defer cuda.Recycle(a2int)
	defer cuda.Recycle(factor)
	defer cuda.Recycle(factor1)
	defer cuda.Recycle(dp)
	cuda.Zero(dp)
	cuda.Zero(factor)
	cuda.Zero(factor1)
	...
	for ii := 0; ii < A1.NComp(); ii++ {
		cuda.Madd2(a1int, A1.Comp(ii), A2.Comp(ii), float32(0.5), float32(0.5))
		cuda.Madd2(a2int, A1.Comp(ii), A2.Comp(ii), float32(0.5), float32(-0.5))
		cuda.Mul(factor1, a2int, dp)
		if ToMulFactor {
			cuda.Madd2(factor, a1int, factor1, float32(float64(1.0)/xArea[ii]), float32(float64(1.0)/xArea[ii]))
			cuda.Mul(Jcurr.Comp(ii), Vappl.Comp(ii), factor)
		} else {
			cuda.Madd2(factor, a1int, factor1, float32(xArea[ii]), float32(xArea[ii]))
			cuda.Div(Jcurr.Comp(ii), Vappl.Comp(ii), factor)
		}
	}
}
